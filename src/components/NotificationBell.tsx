...
const NotificationBell = () => {
  ...
  const handleClick = (notification: Notification) => {
    ...
    if (notification.ticket_id) { navigate(`/tickets/${notification.ticket_id}`); setOpen(false); }
  };
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative h-9 w-9">
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#E8A020] px-1 text-[10px] font-bold text-white">
              ...
              {notifications.map((n) => (
                <button key={n.id} onClick={() => handleClick(n)}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground/60 mt-1">{formatDistanceToNow(new Date(n.created_at), { addSuffix: true })}</p>
                  </div>
